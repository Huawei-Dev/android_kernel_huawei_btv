...
int wifi_init_proc()
{
	...
	struct file* fp;
	fp = filp_open("/proc/wifi",  O_RDONLY, 0);
	if (IS_ERR_OR_NULL(fp)) {
		WL_ERR(("[bcmdhd] proc/wifi is empty\n"));
...
#ifdef WL_TEM_CTRL
	fp = filp_open("/proc/wifi/wifi_tem_stat",  O_RDONLY, 0);
	if (IS_ERR_OR_NULL(fp)) {
		WL_ERR(("[bcmdhd] proc/wifi/wifi_tem_stat is empty %d\n"));
...
#endif
#ifdef WL_TIM_EVENT
	fp = filp_open("/proc/wifi/wifi_tim_stat",  O_RDONLY, 0);
	if (IS_ERR_OR_NULL(fp)) {
		WL_ERR(("[huangpei] proc/wifi/wifi_tim_stat is empty\n"));
